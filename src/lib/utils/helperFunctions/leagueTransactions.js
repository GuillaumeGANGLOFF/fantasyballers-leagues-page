...
import { transactionsStore, leagueID } from '$lib/stores';
...
export const getLeagueTransactions = async (preview, refresh = false) => {
	...
	const {transactionsData, currentSeason} = await combThroughTransactions(week, id).catch((err) => { console.error(err); });
